...
function getTintedGlyph(imgKey, img, color) {
...
export default function CanvasRenderer({
    ...
    useEffect(() => {
        ...
        if (showPageNumbers) {
            ...
            ctx.fillText(`PAGE ${pageIndex + 1} OF ${totalPages}`, baseWidth / 2, baseHeight - 24);
            ctx.restore();
        }
    ...
    function drawPaperBackground(ctx, material, seed) {
        if (material === 'aged') {
    ...
    function drawTextLine(
        ...
        const layoutWords = words.map((word, wIdx) => {
            ...
            const wordRotation =
                (getDeterminRandom(wordSeed + '-r') - 0.5) * 2 * maxRotationRad * (jitter / 6);
            ...
            for (let cIdx = 0; cIdx < word.length; cIdx++) {
                ...
                const baseJitterRange = customGlyphsMetadata
                    ...
                    : 1 + (jitter / 6) * 1.5;
                const charJitterY =
                    (getDeterminRandom(charSeed + '-jy') - 0.5) * 2 * baseJitterRange;

                // 2. Horizontal Spacing Noise (±1.5px max)
                const spacingNoiseRange = (jitter / 6) * 1.5;
                const charSpacingNoise =
                    (getDeterminRandom(charSeed + '-sx') - 0.5) * 2 * spacingNoiseRange;
                ...
                if (urls && urls.length > 0) {
                    chosenVariantIndex = Math.floor(
                        getDeterminRandom(charSeed + '-variant') * urls.length,
                    );
                    ...
                    if (img) {
                        ...
                        charWidth = Math.max(
                            2,
                            (img.width / img.height) * glyphHeight + letterSpacing,
                        );
                    } else {
        ...
        if (!isHeader) {
            if (textAlign === 'center') {
                startX =
                    marginLeft + (textWidthLimit - totalLineWidth) / 2 + indent * (fontSize * 0.4);
            } else if (textAlign === 'right') {
                startX = rightLimit - totalLineWidth;
            } else if (
                textAlign === 'justify' &&
                words.length > 1 &&
                totalLineWidth < textWidthLimit * 0.9
            ) {
        ...
        layoutWords.forEach((wordLayout, wIdx) => {
            ...
            wordLayout.charSpecs.forEach((charLayout, cIdx) => {
                ...
                lineDriftY +=
                    (getDeterminRandom(charSeed + '-drift') - 0.5 + lineBias) *
                    0.7 *
                    (1 + jitter * 0.2);
    ...
    function drawPaperTextureOverlay(ctx, material) {
