...
import type { LineData, PageData } from '../types';
...
const PAPERS: { id: string; name: string; css: string; lineHeight: number; style?: React.CSSProperties }[] = [
    { id: 'plain', name: 'Plain White', css: 'bg-white', lineHeight: 32, style: {} },
    { id: 'lined', name: 'Ruled Notebook', css: 'bg-white', lineHeight: 32, style: { backgroundImage: 'linear-gradient(#e5e7eb 1px, transparent 1px)', backgroundSize: '100% 32px' } },
    { id: 'dotted', name: 'Dot Grid', css: 'bg-white', lineHeight: 32, style: { backgroundImage: 'radial-gradient(#d1d5db 1px, transparent 1px)', backgroundSize: '32px 32px' } },
    { id: 'aged', name: 'Aged Paper', css: 'bg-[#f4ebd0]', lineHeight: 32, style: { backgroundColor: '#f4ebd0' } },
];
...
export default function EditorPage() {
    ...
    const { 
        ...
        customGlyphs, setCustomGlyphs,
        customGlyphsMetadata, setCustomGlyphsMetadata,
    ...
    const paper = useMemo(() => {
        const mappedId = paperMaterial === 'white' ? 'plain' : 
                         paperMaterial === 'ruled' ? 'lined' : 
                         paperMaterial === 'dotted' ? 'dotted' : 
                         paperMaterial === 'aged' ? 'aged' : 'lined';
        const p = PAPERS.find(p => p.id === mappedId) || PAPERS[1];
        return p;
    }, [paperMaterial]);

    const setPaper = (p: { id: string }) => {
        const mappedMaterial = p.id === 'plain' ? 'white' : 
                              p.id === 'lined' ? 'ruled' : 
                              p.id === 'dotted' ? 'dotted' : 
                              p.id === 'aged' ? 'aged' : 'ruled';
        setPaperMaterial(mappedMaterial as any);
    };
    ...
    const handleGlyphsUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            addToast(`Handwriting glyphs extracted (calibrated slant: ${metadata.measuredSlant.toFixed(1)}°)! ✨`, "success");
        } catch (err) {
                        ...
                        <div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div><span className="text-[10px] font-bold text-neutral-400 uppercase tracking-tighter mb-2 flex justify-between">Letter Spacing <span>{letterSpacing}px</span></span><input type="range" min="-12" max="15" value={letterSpacing} onChange={e=>setLetterSpacing(Number(e.target.value))} className="w-full h-1 bg-black/5 rounded-full appearance-none accent-neutral-900 cursor-pointer"/></div>
                                </div>
                                ...
                                <div className="mt-4 pt-4 border-t border-black/5">
                                    <span className="text-[10px] font-black text-neutral-400 uppercase tracking-widest mb-3 block">Custom Handwriting</span>
                                    {isUploadingGlyphs ? (
                                        ...
                                        <div className="space-y-3">
                                            <div className="flex items-center justify-between p-2.5 bg-emerald-50 border border-emerald-100 rounded-xl">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <span className="text-[9px] font-black text-emerald-800 uppercase tracking-widest">Custom Glyphs Active</span>
                                                </div>
                                                <button 
                                                    onClick={() => { setCustomGlyphs(null); setCustomGlyphsMetadata(null); }}
                                            ...
                                            <div className="grid grid-cols-8 gap-1 p-2 bg-neutral-50 border border-black/5 rounded-xl max-h-20 overflow-y-auto custom-scrollbar">
                                                {Object.entries(customGlyphs).slice(0, 16).map(([char, rawUrls]) => {
                                                 const urls = Array.isArray(rawUrls) ? rawUrls : [rawUrls];
                                                 const url = urls && urls.length > 0 ? urls[0] : '';
                                                 return (
                                                     <div key={char} className="aspect-square bg-white border border-black/5 rounded flex items-center justify-center relative group" title={`Char '${char}' (${urls.length} variants)`}>
                                                         {url && <img src={url} alt={char} className="max-w-[75%] max-h-[75%] object-contain" />}
                                                         <span className="absolute inset-0 flex items-center justify-center text-[7px] font-black text-neutral-400 bg-white/80 opacity-0 group-hover:opacity-100 transition-opacity">{char}</span>
                                                     </div>
                                                 );
                                             })}
                                            </div>
                                        ...
                                        <label className="flex flex-col items-center justify-center p-4 border border-dashed border-neutral-200 rounded-xl bg-neutral-50/50 hover:bg-neutral-50 transition-colors cursor-pointer group">
                                            <input 
                                                ...
                                                className="hidden" 
                                            />
                                            <Download size={14} className="text-neutral-400 group-hover:text-neutral-600 transition-colors mb-1.5" />
                                            <span className="text-[10px] font-bold text-neutral-500 group-hover:text-neutral-700 transition-colors text-center">Upload Handwriting Sheet</span>
                                            <span className="text-[8px] text-neutral-400 text-center mt-0.5">Extracts glyphs using OpenCV</span>
                                        </label>
                                    )}
                                </div>
                            </div>
                ...
                <div className="overflow-y-auto p-5 space-y-5 relative" style={{ maxHeight: 'calc(85vh - 200px)' }}>
                    ...
                    {activeMobileTab === 'design' && (
                        <>
                            ...
                            <div className="bg-white border border-black/5 rounded-2xl p-4 shadow-premium">
                                <label className="text-[10px] font-black uppercase tracking-widest text-neutral-400 mb-3 flex justify-between">
                                    Letter Spacing <span className="text-neutral-900">{letterSpacing}px</span>
                                </label>
                                <input 
                                    type="range" 
                                    min="-12" 
                            ...
                            <div className="bg-white border border-black/5 rounded-2xl p-4 shadow-premium">
                                <label className="text-[10px] font-black uppercase tracking-widest text-neutral-400 mb-3 block">Custom Handwriting</label>
                                {isUploadingGlyphs ? (
                                    ...
                                    <div className="space-y-3">
                                        <div className="flex items-center justify-between p-3 bg-emerald-50 border border-emerald-100 rounded-xl">
                                            <div className="flex items-center gap-2">
                                                <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse" />
                                                <span className="text-[10px] font-black text-emerald-800 uppercase tracking-widest">Custom Glyphs Active</span>
                                            </div>
                                            <button 
                                                onClick={() => { setCustomGlyphs(null); setCustomGlyphsMetadata(null); }}
                                    ...
                                    <label className="flex flex-col items-center justify-center p-4 border border-dashed border-neutral-200 rounded-xl bg-neutral-50/50 hover:bg-neutral-50 transition-colors cursor-pointer group">
                                        <input 
                                            ...
                                            className="hidden" 
                                        />
                                        <Download size={16} className="text-neutral-400 group-hover:text-neutral-600 transition-colors mb-1.5" />
                                        <span className="text-[11px] font-bold text-neutral-500 group-hover:text-neutral-700 transition-colors text-center">Upload Handwriting Sheet</span>
                                        <span className="text-[9px] text-neutral-400 text-center mt-0.5">Scans template using OpenCV</span>
                                    </label>
                                )}
                            </div>
                        </>
